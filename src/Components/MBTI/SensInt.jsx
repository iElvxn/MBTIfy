...
function SensInt({songs, accessToken, setSensIntFinished, isLoading, mbti, topGenres}) {
    ...
    return (
        <>
        {!isLoading ?
            <div className="mbti-container" id="sens-int-container" style={{backgroundColor: "#ffffb7"}}>
                {(sensingLevel.current > intuitionLevel.current) ?
                <>
                    <h1 className="header">You are Observant</h1>
                    <div className="bar">
                        <div className="bar-inner" style={{width: `${sensingLevel.current * 100}%`, backgroundColor: "#E9C080", color: "#ffffb7"}}>{Math.round(sensingLevel.current * 100)}%</div>
                    </div>
                </>
                    :
                <>
                    <h1 className="header">You are Intuitive</h1>
                    <div className="bar">
                        <div className="bar-inner" style={{width: `${intuitionLevel.current * 100}%`, backgroundColor: "#E9C080", color: "#ffffb7"}}>{Math.round(intuitionLevel.current * 100)}%</div>
                    </div>
                </>
                } 
                <p className="sub-text">Observant individuals tend to listen to mainstream music or specific genres. Whereas, Intuitive individuals tend to experiment with their music taste as they explore various genres, and listen to less mainstream tracks.</p>
                <h1 style={{fontSize: "1.4rem", fontWeight: "600"}}>Your Top Genres</h1>
                <div className="top-items">
                    {topGenres.current.map(genre => {
                        return (
                            <div className="genre-container" >
                                <img className="song-img" src={genre.src}></img>
                                <p className="song-name">{genre.name}</p>
                            </div>
                        )
                    })}
                </div>
